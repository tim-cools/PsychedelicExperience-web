...
{
    public class WhenUpdatingADoseTimeOffset : ServiceTestBase<IMediator>,
        IClassFixture<PsychedelicsIntegrationTestFixture>
    {
        private readonly UserId _userId = UserId.New();
        private readonly int _offset = 123;

        private DoseId _doseId;
        private ExperienceId _experienceId;
        private Result _result;

        public WhenUpdatingADoseTimeOffset(PsychedelicsIntegrationTestFixture fixture)
            : base(fixture)
        {
        }
        ...
        {
            var query = new UpdateDoseTimeOffset(_userId, _doseId, _offset);
        ...
        public void ThenTheDoseShouldBeUpdated()
        {
            SessionScope(context =>
            {
                Test.All(_ => _.IsNotNull(context.Session.Load<Dose>(_doseId), (dose, __) => __
                    ...
                    .AreEqual(dose.TimeOffsetSeconds, _offset)));
            });
        }
        ...
        public void ThenTheEventShouldBeStored()
        {
            SessionScope(context =>
            {
                ...
                Test.All(_ => _.IsNotNull(events.LastEventShouldBeOfType<DoseTimeOffsetUpdated>(), (@event, __) => __
                    .AreEqual(@event.DoseId, _doseId, "doseId")
                    .AreEqual(@event.UserId, _userId, "userId")
                    .AreEqual(@event.TimeOffsetSeconds, _offset)));
            });
        }
    }
}