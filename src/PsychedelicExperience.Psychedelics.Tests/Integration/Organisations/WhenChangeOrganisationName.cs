...
{
    public class WhenChangeOrganisationName : ServiceTestBase<IMediator>, IClassFixture<PsychedelicsIntegrationTestFixture>
    {
        private readonly UserId _userId = UserId.New();
        private OrganisationId _organisationId;
        private Result _result;

        public WhenChangeOrganisationName(PsychedelicsIntegrationTestFixture fixture)
            : base(fixture)
        {
        }
        ...
        protected override void When(TestContext<IMediator> context)
        {
            ...
            var query = new ChangeOrganisationName(_organisationId, _userId, new Name("newName"));
        ...
        public void ThenTheAggregateShouldBeUpdated()
        {
            SessionScope(context =>
            {
                ...
                aggregate.Name.ShouldBe(new Name("newName"));
            });
        }
        ...
        public void ThenTheEventShouldBeStored()
        {
            SessionScope(context =>
            {
                ...
                var @event = events.LastEventShouldBeOfType<OrganisationNameChanged>();

                @event.UserId.ShouldBe(_userId);

                @event.OrganisationId.ShouldBe(_organisationId);
                @event.Name.ShouldBe(new Name("newName"));
            });
        }
    }
}