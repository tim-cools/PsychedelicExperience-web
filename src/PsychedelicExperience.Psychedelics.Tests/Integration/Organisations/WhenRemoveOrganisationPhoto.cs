...
{
    public class WhenRemoveOrganisationPhoto : ServiceTestBase<IMediator>, IClassFixture<PsychedelicsIntegrationTestFixture>
    {
        ...
        public WhenRemoveOrganisationPhoto(PsychedelicsIntegrationTestFixture fixture)
            : base(fixture)
        {
        }
        ...
        protected override void When(TestContext<IMediator> context)
        {
            ...
            context.AddOrganisationPhoto(_userId, _organisationId, _photoId, "123.png");

            var query = new RemoveOrganisationPhoto(_organisationId, _userId, _photoId);
        ...
        public void ThenTheAggregateShouldBeUpdated()
        {
            SessionScope(context =>
            {
                ...
                aggregate.Photos.ShouldNotContain(photo => Equals(photo.Id, _photoId));
            });
        }
        ...
        public void ThenTheEventShouldBeStored()
        {
            SessionScope(context =>
            {
                ...
                var @event = events.LastEventShouldBeOfType<OrganisationPhotoRemoved>();

                @event.UserId.ShouldBe(_userId);
                @event.OrganisationId.ShouldBe(_organisationId);
                @event.PhotoId.ShouldBe(_photoId);
            });
        }
    }
}