...
{
    public class WhenRemoveOrganisationTtype : ServiceTestBase<IMediator>, IClassFixture<PsychedelicsIntegrationTestFixture>
    {
        private readonly UserId _userId = UserId.New();
        private OrganisationId _organisationId;
        private Result _result;

        public WhenRemoveOrganisationTtype(PsychedelicsIntegrationTestFixture fixture)
            : base(fixture)
        {
        }
        ...
        protected override void When(TestContext<IMediator> context)
        {
            ...
            _organisationId = context.AddOrganisation(_userId, types: new []{ "Retreat", "Community" });

            var query = new RemoveOrganisationType(_organisationId, _userId, "Retreat");
        ...
        public void ThenTheAggregateShouldBeUpdated()
        {
            SessionScope(context =>
            {
                ...
                aggregate.Types.ShouldNotContain(tag => tag == "Retreat");
        ...
        public void ThenTheEventShouldBeStored()
        {
            SessionScope(context =>
            {
                ...
                var @event = events.LastEventShouldBeOfType<OrganisationTypeRemoved>();

                @event.UserId.ShouldBe(_userId);
                @event.OrganisationId.ShouldBe(_organisationId);
                @event.Type.ShouldBe("Retreat");
            });
        }
    }
}