...
{
       public class WhenUnlinkingAnOrganisation : ServiceTestBase<IMediator>, IClassFixture<PsychedelicsIntegrationTestFixture>
    {
        private readonly UserId _userId = UserId.New();
        private OrganisationId _organisationId;
        private OrganisationId _organisationTargetId;
        private Result _result;

        public WhenUnlinkingAnOrganisation(PsychedelicsIntegrationTestFixture fixture)
            : base(fixture)
        {
        }
        ...
        protected override void When(TestContext<IMediator> context)
        {
            ...
            _organisationTargetId = context.AddOrganisation(_userId);

            var link = new LinkOrganisation(_organisationId, _organisationTargetId, _userId, "Shaman");
            context.Service.ExecuteNowWithTimeout(link).ShouldBeSuccess();

            var query = new UnlinkOrganisation(_organisationId, _organisationTargetId, _userId, "Shaman");
        ...
        public void ThenTheAggregateShouldBeUpdated()
        {
            SessionScope(context =>
            {
                ...
                aggregate.Relations.ShouldBeEmpty();
            });
        }
        ...
        public void ThenTheEventShouldBeStored()
        {
            SessionScope(context =>
            {
                ...
                var @event = events.LastEventShouldBeOfType<OrganisationUnlinked>();

                @event.UserId.ShouldBe(_userId);
                @event.OrganisationId.ShouldBe(_organisationId);
                @event.TargetId.ShouldBe(_organisationTargetId);
            });
        }
    }
}