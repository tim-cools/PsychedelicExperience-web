...
{
    public class WhenChangeEventLocation : ServiceTestBase<IMediator>, IClassFixture<PsychedelicsIntegrationTestFixture>
    {
        private EventId _eventId;
        private ChangeEventLocation _command;
        private UserId _userId;
        private Result _result;

        public WhenChangeEventLocation(PsychedelicsIntegrationTestFixture fixture)
        ...
        protected override void When(TestContext<IMediator> context)
        {
            ...
            _command = new ChangeEventLocation(_userId, _eventId, new EventLocation { Name = new Name("Changed") });
        ...
        public void ThenTheEventShouldBeStored()
        {
            SessionScope(context =>
            {
                ...
                var @event = events.LastEventShouldBeOfType<EventLocationChanged>();

                @event.UserId.ShouldBe(_userId);

                @event.EventId.ShouldBe(_eventId);
                @event.Location.Name.ShouldBe(_command.Location.Name);
            });
        }
    }
}