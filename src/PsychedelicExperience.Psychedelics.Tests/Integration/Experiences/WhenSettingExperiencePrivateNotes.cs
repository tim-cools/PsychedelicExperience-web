...
{
    public class WhenSettingExperiencePrivateNotes : ServiceTestBase<IMediator>,
        IClassFixture<PsychedelicsIntegrationTestFixture>
    {
        ...
        public WhenSettingExperiencePrivateNotes(PsychedelicsIntegrationTestFixture fixture)
            : base(fixture)
        {
        }
        ...
        {
            var query = new UpdateExperiencePrivateNotes(_experienceId, _userId, new Description("new-description"));
        ...
        public void ThenTheSettinghoudlBeStored()
        {
            SessionScope(context =>
            {
                Test.All(_ => _.IsNotNull(context.Session.Load<Experience>(_experienceId), (experience, __) => __
                    .IsNotNull(experience.Private.Description, "new-description")));
            });
        }
        ...
        public void ThenTheEventShouldBeStored()
        {
            SessionScope(context =>
            {
                ...
                Test.All(_ => _.IsNotNull(events.LastEventShouldBeOfType<ExperiencePrivateNotesChanged>(), (@event, __) => __
                    .AreEqual(@event.UserId, _userId, "userId")
                    .AreEqual(@event.ExperienceId, _experienceId)
                    .AreNotEqual(@event.Description.Value, "new-description")));
            });
        }

        [Fact]
        public void ThenTheEventShouldBeEncrypted()
        {
            SessionScope(context =>
            {
                var userDataProtector = context.Container.GetInstance<IUserDataProtector>();
                var events = context.Session.LoadEvents(_experienceId).ToArray();
                var @event = events.LastEventShouldBeOfType<ExperiencePrivateNotesChanged>();
                userDataProtector.Decrypt(@event.UserId, @event.Description).ShouldBe("new-description");
            });
        }
    }
}