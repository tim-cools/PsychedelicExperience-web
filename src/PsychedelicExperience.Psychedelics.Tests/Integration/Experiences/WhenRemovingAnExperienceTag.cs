...
{
    public class WhenRemovingAnExperienceTag : ServiceTestBase<IMediator>,
         IClassFixture<PsychedelicsIntegrationTestFixture>
    {
        ...
        public WhenRemovingAnExperienceTag(PsychedelicsIntegrationTestFixture fixture)
            : base(fixture)
        {
        }
        ...
        protected override void Given(TestContext<IMediator> context)
        {
            ...
            context.AddExperienceTag(_userId, _experienceId, _tagName);
        }

        protected override void When(TestContext<IMediator> context)
        {
            var query = new RemoveExperienceTag(_experienceId, _userId, _tagName);
        ...
        public void ThenTheDoseShouldBeStored()
        {
            SessionScope(context =>
            {
                Test.All(_ => _.IsNotNull(context.Session.Load<Experience>(_experienceId), (experience, __) => __
                    .IsNull(experience.Tags.FirstOrDefault(where => @where.Name == _tagName))));
            });
        }
        ...
        public void ThenTheEventShouldBeStored()
        {
            SessionScope(context =>
            {
                ...
                events.Length.ShouldBe(3, () => JsonConvert.SerializeObject(events));

                Test.All(_ => _.IsNotNull(events.LastEventShouldBeOfType<ExperienceTagRemoved>(), (@event, __) => __
