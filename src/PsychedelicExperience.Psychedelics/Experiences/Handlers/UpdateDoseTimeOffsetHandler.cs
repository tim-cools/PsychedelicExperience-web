...
{
    public class UpdateDoseTimeOffsetHandler : CommandHandler<UpdateDoseTimeOffset, Result>
    {
        public UpdateDoseTimeOffsetHandler(IDocumentSession session) : base(session)
        {
        }

        protected override async Task<Result> Execute(UpdateDoseTimeOffset command)
        {
            var aggregate = await Session.LoadAggregate<Dose>(command.DoseId);

            var user = await Session.LoadUserAsync(command.UserId);

            aggregate.UpdateTimeOffset(user, command.TimeOffsetSeconds);
